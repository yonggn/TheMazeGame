...
namespace Engine
{
    Level2::Level2(GameDataRef data): _data(data)
    {
        scr=0;
    }
    ...
    {
        ifstream readFile;
        readFile.open("Assets/PlayerData/HighestScoreLevel2.txt");
        if(readFile.is_open())
        {
            while(!readFile.eof())
            {
        ...
        cout <<"Elf Texture Loaded\n";
         _data->assets.LoadTexture("BossVillage",BOSS1_TEXTURE_FILEPATH);
         cout<<"Boss Village loaded";
        ...
        //Set up maze
        //Maze maze;
        getMaze(LEVEL2_MAZE_FILEPATH);
        cout<<"Level2.h: "<<endl;

        IntRect rtrap(0,0,32,39);
        IntRect rroulant(0,128,63,64);
        IntRect relf(relfLeft,0,32,32);
        IntRect relfDie(relfLeft,32,32,32);
        IntRect rboss1(0,0,63,78);
                    ...
                    smaze[i][j].setPosition(j*40, i*30);
                        ...
                        else if(maze[i][j]=='a')
                        {
                            if(elfDie==false)
                            {
                                ...
                                elf4->posY=i;
                                elf4->posX=j;
                                elf4->elf.setPosition(j*40,i*30);
                                elf4->elf.setScale(1.0f,1.0f);
                            }
                        }
                        else if(maze[i][j]=='r')
                        {
                            if(elfDie==false)
                            {
                                ...
                                elf5->posY=i;
                                elf5->posX=j;
                                elf5->elf.setPosition(j*40,i*30);
                                elf5->elf.setScale(1.0f,1.0f);
                            }
                        }
                        ...
                        else if(maze[i][j]=='t')
                        {
                            ...
                            smaze[i][j].setPosition(j*39, i*30);
    ...
    {
        ofstream writeFile("Assets/PlayerData/HighestScoreLevel2.txt");
        ...
        if(elf4->elf.getGlobalBounds().intersects(roulant->roulant.getGlobalBounds()))
        {
            if(roulant->isAttack == true)
            {
                elf4->elfDie=true;
                maze[elf4->posY][elf4->posX]='f';
                elf4->elf.setScale(0,0);
                scr+=200;
                cout<<"elf Die\n";
            }
            roulant->isAttack=false;
        }
        if(elf5->elf.getGlobalBounds().intersects(roulant->roulant.getGlobalBounds()))
        {
            if(roulant->isAttack == true)
            {
                elf5->elfDie=true;
                maze[elf5->posY][elf5->posX]='f';
                elf5->elf.setScale(0,0);
                scr+=200;
                cout<<"elf Die\n";
            }
            roulant->isAttack=false;
        ...
        if (timerev > 1.0f)
        {
            ...
            if(elf->elf.getGlobalBounds().intersects(roulant->roulant.getGlobalBounds())
               ...
               ||elf4->elf.getGlobalBounds().intersects(roulant->roulant.getGlobalBounds())
               ||elf5->elf.getGlobalBounds().intersects(roulant->roulant.getGlobalBounds()))
            {
            ...
            //deduct lives when player touches boss
            if(boss->boss.getGlobalBounds().intersects(roulant->roulant.getGlobalBounds()))
            {
                roulant->playerOuch=true;
            ...
            //deduct live when touch the wrong exit
        ...
        if(timere>0.5f)
        {
            ...
            elf2->randomMove(elfMove2,maze);
            elf3->randomMove(elfMove3,maze);
            elf4->randomMove(elfMove4,maze);
            elf5->randomMove(elfMove5,maze);
            boss->randomMove(elfMove6,maze);
            timere=0;
        ...
        if(lvlComplete)
        {
            if(scr>=300)
            {
                ...
                this->_data->stateManager.AddState(StateRef (new EndScreen1(this->_data,this->scr,this->highestScr)));
                lvlComplete=false;
            }
            else
                lvlComplete=false;
        ...
        {
            cout<<"Lose";
            this->_data->stateManager.AddState(StateRef (new GameOver1(_data)));
    ...
    void Level2::Draw(float deltaTime)
    {
