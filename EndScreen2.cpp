...
namespace Engine
{
    EndScreen2::EndScreen2(GameDataRef data, int score, int highestScore): _data(data),_score(score),_highestScore(highestScore)
    {

    }

    void EndScreen2::Init()
    {
        cout<<"end scene\n";
    ...
    void EndScreen2::Update(float dt){}

    void EndScreen2::Draw(float dt)
    {
        ...
        {
            this->_data->window.clear(Color::Black);
            this->_data->stateManager.AddState(StateRef (new SplashState3(this->_data)),true);
            cout<<"ToLevel2Splash\n";
        }
